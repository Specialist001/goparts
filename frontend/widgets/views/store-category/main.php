...
if (!function_exists('getMenuChild')) {
    function getMenuChild($child, $index = 0)
    {
        ...
        foreach ($child as $item) {
            ...
            $title = /*$img.*/
                '<span style="display: table-cell;vertical-align: middle;width: 95%;">' . $item->translate->title . '</span>';
...
<?php if (!empty($menu)) {
    ...
    <div class="tab-content">
        <?php for ($lcm = 0; $lcm < count($menu); $lcm++) {
            if ($lcm == 0) $active_top_cat = $menu[$lcm]->id; ?>
            <div role="tabpanel"
                 class="tab-pane <?= ($active_top_cat == $menu[$lcm]->id) ? 'active tab-pane-first' : '' ?>"
                 id="tab-menu<?= ($lcm + 1 + $tab) ?>">
