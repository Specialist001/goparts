...
class WCategory extends Widget
{
    ...
    public function run() {
        ...
        if($this->key == 'preview') {
            return $this->render('store-category/preview', [
                'menu' => Category::find()->where(['status' => 1, 'on_main' => 1])->orderBy('order')->all()
            ]);
        }
        if($this->key == 'main') {
            $lang = Language::getCurrent()->locale;
            $menu_main = Yii::$app->cache->get('menu_main_'.$lang);

            if($menu_main === false) {
                $menu_main = StoreCategory::find()->with('activeCategories')->with('translate')->where(['status' => 1, 'parent_id' => null])->orderBy('order')->all();
            ...
            if($menu_main_render === false) {
                $menu_main_render =  $this->render('store-category/main', [
                    'menu' => $menu_main,
                    'tab' => $this->tab
                ]);
